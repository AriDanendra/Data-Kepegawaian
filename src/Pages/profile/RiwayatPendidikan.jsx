...
const RiwayatPendidikan = ({ data: propData, employeeId: propEmployeeId }) => {
  ...
  const handleSaveChanges = async (e) => {
    ...
    try {
      let response;
      if (modalType === 'add') {
        response = await axios.post(`http://localhost:3001/api/employees/${employeeId}/pendidikan`, dataToSend, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
        setPendidikanData([...pendidikanData, response.data]);
        showSuccessModal(`Data pendidikan baru berhasil ditambahkan!`);
      } else {
        response = await axios.put(`http://localhost:3001/api/employees/${employeeId}/pendidikan/${selectedData.id}`, dataToSend, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
