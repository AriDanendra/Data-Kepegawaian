...
const RiwayatSKPPermenpan = ({ data: propData, employeeId: propEmployeeId }) => {
  ...
  const handleSaveChanges = async (e) => {
    ...
    try {
      let response;
      if (modalType === 'add') {
        response = await axios.post(`http://localhost:3001/api/employees/${employeeId}/skp-permenpan`, dataToSend, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
        setSkpData([...skpData, response.data]);
        showSuccessModal(`Data SKP Permenpan baru berhasil ditambahkan!`);
      } else {
        response = await axios.put(`http://localhost:3001/api/employees/${employeeId}/skp-permenpan/${selectedData.id}`, dataToSend, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
