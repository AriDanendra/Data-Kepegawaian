...
const DataKeluarga = ({ data: propData, employeeId: propEmployeeId }) => {
  ...
  const handleOpenModal = (type, data = null) => {
    ...
    if (type.startsWith("edit")) {
      ...
      if (type === "add-pasangan")
        setFormData({ kategori: "pasangan", nama: "", ttl: "", tglKawin: "" });
      if (type === "add-orangtua")
        setFormData({
          kategori: "orangtua",
          nama: "",
          status: "",
      ...
      if (type === "add-anak")
        setFormData({ kategori: "anak", nama: "", ttl: "", pendidikan: "" });
    } else {
      setSelectedData(data);
    }
    setIsModalOpen(true);
    ...
    try {
      ...
      if (modalType.startsWith("add")) {
        response = await axios.post(
          `/api/employees/${employeeId}/keluarga`,
          dataToSend,
          {
        ...
        response = await axios.put(
          `/api/employees/${employeeId}/keluarga/${selectedData.id}`,
          dataToSend,
          {
            headers: { "Content-Type": "multipart/form-data" },
          }
  ...
  const handleDelete = async () => {
    try {
      ...
      setKeluargaData(
        keluargaData.filter((item) => item.id !== selectedData.id)
      );
      ...
      return (
        <div>
          <p>
            ...
            <strong>{selectedData.nama}</strong>?
          </p>
          <div className="modal-form-actions">
            <button
              type="button"
          ...
          <td>
            <div className="action-buttons">
              <button
                ...
                title="Edit"
                onClick={() => handleOpenModal(`edit-${type}`, item)}
              >
                <FaPencilAlt />
              </button>
              <button
                ...
                title="Delete"
                onClick={() => handleOpenModal(`delete-${type}`, item)}
              >
                <FaTrash />
              </button>
            </div>
